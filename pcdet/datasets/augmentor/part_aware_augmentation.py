...
class PartAwareAugmentation(object):
    def __init__(self, points, gt_boxes, gt_names, class_names=None, random_partition=False):
        ...
        self.num_partition['Bus'] = 8
        self.num_partition['Truck'] = 8
        self.num_partition['Pedestrian'] = 4
        self.num_partition['Cyclist'] = 4
        self.num_classes = len(class_names)
        self.random_partition = random_partition
        # self.cls_to_idx = {'Car': 0, 'Pedestrian': 1, 'Cyclist': 2}
        self.cls_to_idx = {key: id for id, key in enumerate(class_names)}
        if random_partition:
    ...
    # new add
    def interpret_pa_aug_param_new(self, pa_aug_param):
        """
        e.g., paa:dropout:02:swap:02:mix:02:sparse40:01:noise10:01
        :param pa_aug_param:
        - 支持组合不同的增强策略；例如dropout:02表示其中的一种策略，默认每个类别使用统一的增强测量
        - 支持只对感兴趣的类别进行增强；例如有三个类别，只想对后两个类别增强；paa011:dropout:02:swap:02，其中1表示使能
        - 支持对不同的类别使用不同增强策略；例如有三个类别，dropout只对前一个类别有效，swap对后两个列别有效；paa:dropout:02:100:swap:02:011
        :return:
        """
        pa_aug_param_dict = {}

        # 判断是否有全局的类别mask
        head = pa_aug_param.split(':')[0]
        pa_aug_param = ':'.join(pa_aug_param.split(':')[1:])
        cls_mask = [True] * self.num_classes
        if len(head) > 3:
            cls_mask = [True if t == '1' else False for t in head[3:]]
        method_list = ['dropout', 'sparse', 'noise', 'swap', 'mix', 'jitter', 'random', 'distance']
        for method in method_list:
            if method == 'distance':
                pa_aug_param_dict[method] = 100
            ...
            if method == 'random':
                pa_aug_param_dict[method] = False
                continue
            pa_aug_param_dict[method] = {
                'num': 0, 'proportion': 0, 'cls': cls_mask
            }

        valid_method_list = [method for method in method_list if method in pa_aug_param]
        str_idx = [pa_aug_param.find(method) for method in valid_method_list] + [len(pa_aug_param)]
        str_idx.sort()
        method_str_list = [pa_aug_param[str_idx[i]:str_idx[i+1]] for i in range(len(valid_method_list))]
        for method_str in method_str_list:
            method_str = method_str[:-1] if method_str[-1] == ':' else method_str
            attr = method_str.split(":")
            name = [method for method in valid_method_list if attr[0].startswith(method)][0]
            sub_str = attr[0][len(name):]
            pa_aug_param_dict[name]['num'] = int(sub_str) if len(sub_str) else 1
            pa_aug_param_dict[name]['proportion'] = float(attr[1]) / 10
            if len(attr) == 3:
                assert len(attr[2]) == self.num_classes
                pa_aug_param_dict[name]['cls'] = [True if i == '1' else False for i in attr[2]]
    ...
    #####################################
    def dropout_partitions(self, num_dropout_partition=4, distance_limit=100, p=1.0, cls=None, gt_box_idx=None, robustness_test=False):
        ...
        for i in range(self.num_gt_boxes):
            if gt_box_idx is not None and i != gt_box_idx:
                continue
            if cls is not None:
                if not cls[self.cls_to_idx[self.gt_names[i]]]:
                    continue
    ...
    #######################################
    def swap_partitions(self, num_swap=1, distance_limit=100, p=1.0, cls=None, gt_box_idx=None):
        if num_swap <= 0:
            return
    ...
    def swap_partitions_random(self, num_swap=1, distance_limit=100, p=1.0, cls=None, gt_box_idx=None):
        if num_swap <= 0:
            return
    ...
    #######################################
    def mix_partitions(self, num_mix=1, distance_limit=100, p=1.0, cls=None, gt_box_idx=None):
        if num_mix <= 0:
            return
    ...
    def mix_partitions_random(self, num_mix=1, distance_limit=100, p=1.0, cls=None, gt_box_idx=None):
        if num_mix <= 0:
            return
    ...
    #############################
    def make_points_sparse(self, num_points_limit=1000, distance_limit=100, p=1.0, cls=None, FPS=False, gt_box_idx=None):
    ...
    ########################################
    def jittering(self, sigma=0.01, p=0.5, cls=None, distance_limit=100, gt_box_idx=None):
        for i in range(self.num_gt_boxes):
            if gt_box_idx is not None and i != gt_box_idx:
                continue
            if cls is not None:
    ...
    ###################################################
    def generate_random_noise(self, num_points=30, distance_limit=100, p=0.5, cls=None, gt_box_idx=None):
        if num_points <= 0:
            return
    ...
    def generate_random_noise_random(self, num_points=30, distance_limit=100, p=0.5, cls=None, gt_box_idx=None):
        if num_points <= 0:
            return
    ...
    def augment_new(self, pa_aug_param):
        pa_aug_param = self.interpret_pa_aug_param_new(pa_aug_param)
        self.dropout_partitions(
            num_dropout_partition=pa_aug_param['dropout']['num'], p=pa_aug_param['dropout']['proportion'],
            cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
        if self.random_partition:
            self.swap_partitions_random(num_swap=pa_aug_param['swap']['num'], p=pa_aug_param['swap']['proportion'],
                                        cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
            self.mix_partitions_random(num_mix=pa_aug_param['mix']['num'], p=pa_aug_param['mix']['proportion'],
                                       cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
        else:
            self.swap_partitions(num_swap=pa_aug_param['swap']['num'], p=pa_aug_param['swap']['proportion'],
                                 cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
            self.mix_partitions(num_mix=pa_aug_param['mix']['num'], p=pa_aug_param['mix']['proportion'],
                                cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
        self.make_points_sparse(num_points_limit=pa_aug_param['sparse']['num'], p=pa_aug_param['sparse']['proportion'],
                                cls=pa_aug_param['dropout']['cls'], FPS=True, distance_limit=pa_aug_param['distance'])
        self.jittering(sigma=pa_aug_param['jitter']['num'], p=pa_aug_param['jitter']['proportion'],
                       cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
        if self.random_partition:
            self.generate_random_noise_random(num_points=pa_aug_param['noise']['num'], p=pa_aug_param['noise']['proportion'],
                                              cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
        else:
            self.generate_random_noise(num_points=pa_aug_param['noise']['num'], p=pa_aug_param['noise']['proportion'],
                                       cls=pa_aug_param['dropout']['cls'], distance_limit=pa_aug_param['distance'])
