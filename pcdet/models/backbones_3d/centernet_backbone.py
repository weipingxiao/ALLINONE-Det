...
from ...utils.spconv_utils import replace_feature, spconv  # 兼容spconv v1 v2
...
class SparseConvolution(SparseModule):
    ...
    def forward(self, input):
        ...
        if not self.subm:
            if self.transposed:
                out_spatial_shape = spconv.ops.get_deconv_output_size(
        ...
        if self.subm:
            out_features = spconv.functional.indice_subm_conv(features, self.weight,
                                              indice_pairs.to(device),
                                              indice_pair_num,
                                              outids.shape[0])
        else:
            if self.inverse:
                out_features = spconv.functional.indice_inverse_conv(features,
                                            self.weight, indice_pairs.to(device),
                                            indice_pair_num, outids.shape[0])
            else:
                out_features = spconv.functional.indice_conv(features,
                                            self.weight, indice_pairs.to(device),
                                            indice_pair_num, outids.shape[0])
