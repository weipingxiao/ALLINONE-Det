...
class CenterHead(nn.Module):
    ...
    def assign_targets(self, gt_boxes):
        ...
        def to_cpu_list_(tensor):
            return [[t[0].cpu()] for t in tensor]
        ...
        try:
            ...
            heatmaps = np.array(to_cpu_list_(heatmaps)).transpose(1, 0).tolist()
            heatmaps = [torch.stack(hms_).cuda() for hms_ in heatmaps]
            anno_boxes = np.array(to_cpu_list_(anno_boxes)).transpose(1, 0).tolist()
            anno_boxes = [torch.stack(anno_boxes_).cuda() for anno_boxes_ in anno_boxes]
            inds = np.array(to_cpu_list_(inds)).transpose(1, 0).tolist()
            inds = [torch.stack(inds_).cuda() for inds_ in inds]
            masks = np.array(to_cpu_list_(masks)).transpose(1, 0).tolist()
            masks = [torch.stack(masks_).cuda() for masks_ in masks]
    ...
    def get_cls_layer_loss(self):
        ...
        cls_loss = self.loss_cls(
            pred_heatmaps, gt_heatmaps, avg_factor=max(num_pos, 1))
